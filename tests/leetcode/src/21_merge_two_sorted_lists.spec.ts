...
import { ListNode, MergeTwoSortedLists } from "../../../packages/leetcode/src/21_merge_two_sorted_lists.js";
...
describe('MergeTwoSortedLists', () => {
  ...
  // Quarantined: the input breaks the LeetCode 21 precondition that both lists are sorted in non-decreasing order, so no output is defined. Kept, not deleted, until the intended behaviour is decided.
  it.skip('should merge two lists with one unsorted list', () => {
    // Given
    const list1 = new ListNode(1);
    list1.next = new ListNode(3);
    list1.next.next = new ListNode(5);
  ...
  // Quarantined: the input breaks the LeetCode 21 precondition that both lists are sorted in non-decreasing order, so no output is defined. Kept, not deleted, until the intended behaviour is decided.
  it.skip('should merge two lists with both unsorted lists', () => {
    // Given
  ...
  // Quarantined: the input breaks the LeetCode 21 precondition that both lists are sorted in non-decreasing order, so no output is defined. Kept, not deleted, until the intended behaviour is decided.
  it.skip('should merge two non-empty sorted lists with one list containing duplicate values at the beginning', () => {
    // Given
    const list1 = new ListNode(5);
    list1.next = new ListNode(3);
    list1.next.next = new ListNode(1);
